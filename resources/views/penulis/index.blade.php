        ...
        <table class="table table-hover mb-0">
            <thead>
                <tr style="background:#fafafa;">
                    ...
                    <th class="px-3 py-2" style="font-size:11px;color:#666;text-transform:uppercase;">Username</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="px-3 py-2" style="font-size:13px;color:#666;">{{ $item->user_name }}</td>
                    <td class="px-3 py-2">
                        <div class="d-flex gap-2">
                            <a href="{{ route('penulis.edit', $item->id) }}" class="btn btn-sm"
                                style="background:#e3f2fd;color:#1565c0;font-size:12px;">Edit</a>
                            <form action="{{ route('penulis.destroy', $item->id) }}" method="POST"
                                onsubmit="return confirm('Hapus penulis ini?')">
                                @csrf @method('DELETE')
                                <button type="submit" class="btn btn-sm"
                                    style="background:#ffebee;color:#c62828;font-size:12px;">Hapus</button>
                            </form>
                        </div>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="4" class="px-3 py-4 text-center text-muted" style="font-style:italic;">Belum ada data penulis.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
